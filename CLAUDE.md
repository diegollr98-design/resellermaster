...
**SIGUIENTE PASO: EL GATE REAL, que sigue sin hacerse — Diego exporta UN producto de verdad, lo pega en Wallapop, y CRONOMETRA (baseline ~285 s/producto).** Es la única métrica que dice si todo esto ahorra tiempo (`decision-making §19`); lleva pendiente varias fases. Otros cabos: precio en **Vinted** (búsqueda tras Datadome), el `test_curar` flaky (timeout `AppTest`), y `género`→título/descripción si Diego lo quiere ahí. Descartado y medido, **no reabrir**: búsqueda por imagen/Lens, visión en el agrupado, composición por búsqueda externa.
