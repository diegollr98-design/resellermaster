...
## v0.7 | Actualizado: 2026-07-17 | App local (Streamlit): fotos de reventa en masa → agrupar por producto → campos de Wallapop y Vinted → copiar-pegar.
...
**FASE 3 CERRADA ✅ (el EXPORT) — commit `c1d0c73`.** `core/export.py` + `ui/export.py` (pestaña "4. Export"): traduce estado a los literales exactos por plataforma (con guarda de traducción-con-signo, `§16`), bloquea ficha sin confirmar / marca ajena / email / enlace, fotos por plataforma. Extraer TODO el lote de una (14 clics → 2); confirmar todas las fichas de golpe SIN mentir la procedencia; desperfectos desde marcador. El export era el **~66% del tiempo de Diego** (~285 s/producto medido por un panel de 4 agentes) y no existía; ahora existe.

**SIGUIENTE PASO: FASE 4 — pulir el export + PRECIO por comparables, en sesión FRESCA con `docs/seeds/fase-4-precio-y-pulido-export.md`.** Dos frentes: (1) **quitar la fricción EVITABLE del export** — categoría (proponer 2-3 hojas candidatas desde los árboles PÚBLICOS de Wallapop/Vinted, NUNCA auto-elegir la hoja: una hoja mal = anuncio oculto), `package_size`/`tramo_peso` (chip de envío sugerido, sesgado hacia arriba), ubicacion (config-una-vez). (2) **el PRECIO v2**: mediana de N parecidos por TEXTO (marca+tipo+talla, nunca EAN) leyendo la búsqueda pública — **corre el Loop de Cambios y que Diego confirme el approach de scraping ANTES de escribir una línea** (`architecture.md`: LEER≠PUBLICAR, volumen doméstico, sin stealth, nada de escribir). **El GATE real pendiente desde Fase 3:** Diego exporta un producto de verdad, lo pega en Wallapop y CRONOMETRA (baseline ~285 s) — sin ese número no sabemos si algo ahorra tiempo. Descartado y medido, **no reabrir**: búsqueda por imagen/Lens, visión en el agrupado, composición por búsqueda externa.
