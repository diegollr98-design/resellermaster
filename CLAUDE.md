...
## v0.10 | Actualizado: 2026-07-21 | App local (Streamlit): fotos de reventa en masa → agrupar por producto → campos de Wallapop y Vinted → copiar-pegar.
...
**SIGUIENTE PASO: FASE 5 — FINANZAS, en sesión FRESCA con `docs/seeds/fase-5-finanzas.md`.** 5ª pantalla: registro de ventas + beneficio + export a Excel. Botón "Subido" por plataforma en Export, **número de referencia** único (generado en Ficha, impreso en las descripciones = la llave para localizar la venta), campo **coste** manual en Ficha (opcional), marcar "Vendido" (precio final auto+editable + plataforma), beneficio. Superficie SENSIBLE de **PERSISTENCIA** (ventas = dinero). El seed lleva los REQUISITOS de Diego (fijos) + los HALLAZGOS de un panel de 4 agentes (candidatos) e instruye **RE-AUDITARLOS con un panel multi-agente** (PASO 0, mínimo sesgo) antes de tocar nada — incluidos 3 riesgos de corrupción del modelo de datos en `store.py`. **El GATE REAL sigue pendiente y es ORTOGONAL** (Diego cronometra un export, ~285 s — la métrica que dice si esto ahorra tiempo, `§19`). Otros cabos: precio en **Vinted** (Datadome), `test_curar` flaky. Descartado y medido, **no reabrir**: búsqueda por imagen/Lens, visión en el agrupado, composición por búsqueda externa.
