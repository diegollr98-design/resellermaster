...
## v0.9 | Actualizado: 2026-07-21 | App local (Streamlit): fotos de reventa en masa → agrupar por producto → campos de Wallapop y Vinted → copiar-pegar.
...
**SIGUIENTE PASO: el `tipo` de producto en la EXTRACCIÓN, en sesión FRESCA con `docs/seeds/fase-4-tipo-producto.md`.** Hueco que detectó Diego usando la app: la síntesis produce marca/talla/color/estado/categoría/título pero **NO qué ES el producto** ("masajeador de rodilla", "sudadera", "jersey"). Es fundamental para TRES cosas: el **título**, el **precio** (la palabra clave de búsqueda de comparables) y las **candidatas de categoría**. Superficie SENSIBLE (`core/extract.py`) → Loop de Cambios + `listing-audit` + `/eval`. Decisión abierta: ¿`tipo` enum cerrado o texto corto editable (recomendado)? ¿y el género? **El GATE real sigue pendiente:** Diego exporta de verdad y CRONOMETRA (baseline ~285 s). Descartado y medido, **no reabrir**: búsqueda por imagen/Lens, visión en el agrupado, composición por búsqueda externa.
