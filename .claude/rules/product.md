...
- Vinted: enums completos de `package_size`, colores y *size groups* **numéricos** (`id`) requieren token de la API Pro de ontologías. **OJO — corrección factual (2026-07-17, `[INC-016]`/seed fase-4):** el **ÁRBOL de categorías** (`catalogTree`, 2482 hojas) **SÍ es obtenible sin token** del SSR de `www.vinted.es` (cookie anónima, 200); y sus **etiquetas** de `package_size` (Pequeño/Mediano/Grande/pesado, `vinted.com/help/51`) son públicas — la app copia-pega en el formulario, no publica por API, así que le basta la etiqueta, no el `id`. "Requiere token" es cierto **sólo** para el `id` numérico, no para el árbol ni las etiquetas. El árbol varía por país+divisa y está versionado por fecha → job de descarga **periódico**, no congelar para siempre.
